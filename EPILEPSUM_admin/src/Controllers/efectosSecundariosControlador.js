...
efectosSecundariosCtl.enviar = async (req, res) => {
	...
	const { nombreEfectosSecundarios, descripcionEfectosSecundarios } =
		req.body;
	const nuevoEnvio = {
...
efectosSecundariosCtl.lista = async (req, res) => {
	...
	const lista = await baseDatosSQL.query(
		"select * from  efectosSecundarios "
	);
...
efectosSecundariosCtl.traer = async (req, res) => {
	const ids = req.params.id;
	const lista = await baseDatosSQL.query(
		"select * from  efectosSecundarios where idEfectosSecundarios = ?",
		[ids]
	);
...
efectosSecundariosCtl.actualizar = async (req, res) => {
	...
	await baseDatosORM.efectosSecundarios
		.findOne({ where: { idEfectosSecundarios: ids } })
		.then((actualizar) => {
