...
usuarios.lista = async (req, res) => {
  ...
  const enlistar = await baseDatosSQL.query(
    "SELECT * FROM rolusuarios WHERE usuarioIdUsuario= ?",
    [usuariosId]
  );
...
usuarios.traerDatos = async (req, res) => {
  ...
  const enlistar = await baseDatosSQL.query(
    "SELECT * FROM rolusuarios WHERE idusuario = ?",
    [usuariosId]
  );
...
usuarios.editar = async (req, res) => {
  ...
  await baseDatosORM.rolUsuarios
    .findOne({ where: { idusuario: usuariosId } })
    .then((usuarios) => {
...
usuarios.eliminar = async (req, res) => {
  const usuariosId = req.params.id;
  const id = req.user.idUsuario;
  await baseDatosORM.rolUsuarios.destroy({
    where: { idusuario: usuariosId },
  });
  res.redirect("/usuarios/lista/" + id);
};
