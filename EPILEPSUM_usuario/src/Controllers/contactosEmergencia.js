...
contactosEmergencia.agregar = async (req, res) => {
    ...
    const {nombreContactosEmergencia, familiar, telefonoContactosEmergencia, celularContactosEmergencia} = req.body
...
contactosEmergencia.lista = async (req, res) => {
    const contactosEmergenciaId = req.params.id;
    const enlistar = await baseDatosSQL.query("SELECT c.*, f.* FROM contactosEmergencia c JOIN familiares f ON f.idFamiliar = c.familiareIdFamiliar WHERE c.pacienteIdPaciente = ?",[contactosEmergenciaId])
...
contactosEmergencia.editar = async (req, res) => {
    ...
    await baseDatosORM.contactosEmergencia.findOne({where: {idContactosEmergencia: contactosEmergenciaId}})
    .then(contactos => {
...
contactosEmergencia.eliminar = async (req, res) => {
    const contactosEmergenciaId = req.params.id;
    const id = req.user.idPaciente
    await baseDatosORM.contactosEmergencia.destroy({where: {idContactosEmergencia: contactosEmergenciaId}})
    res.redirect("/contactos/lista/" + id);
}
